import { UnitSelect } from '@/common/open-weather/components'
...
describe('<ForecastWidget />', () => {
  ...
  function createWrapper() {
    return shallow(
      <ForecastWidget.wrappedComponent
  ...
  describe('navigator.geolocation is defined', () => {
    ...
    describe('renderGetForecastButton', () => {
      it('renders a <CircularProgress /> when forecast is in progress', () => {
        openWeatherStore.forecastInProgress = true
        wrapper = createWrapper()
        const circularProgress = wrapper.find(CircularProgress)
        expect(circularProgress.exists()).toBeTruthy()
      })

      it('renders a get forecast <Button /> when forecast is not in progress', () => {
        openWeatherStore.forecastInProgress = false
        wrapper = createWrapper()
        const button = wrapper.find(Button)
        expect(button.exists()).toBeTruthy()
        expect(button.children().text()).toEqual('Get Forecast')
      })
    })

    describe('renderForecastBlock', () => {
      it('renders a list of <Typography /> for temperature and <Weather /> when forecast succeeded and is defined', () => {
        openWeatherStore.forecastInProgress = false
        openWeatherStore.forecastSucceeded = true
        openWeatherStore.forecastData = mockForecastData
        wrapper = createWrapper()
        const typography = wrapper.find(Typography)
        const expectedTypographyCount = 1 + 2 * mockForecastData.list.length
        expect(typography).toHaveLength(expectedTypographyCount)
        expect(
          typography
            .at(0)
            .children()
            .text()
        ).toEqual('Forecast for your area:')
      })

      it('renders a disclaimer when forecast fails', () => {
        openWeatherStore.forecastInProgress = false
        openWeatherStore.forecastFailed = true
        wrapper = createWrapper()
        const typography = wrapper.find(Typography)
        expect(typography.exists()).toBeTruthy()
        expect(typography.children().text()).toEqual(forecastFailed)
      })

      it('renders null when forecast neither succeeded nor failed', () => {
        expect(wrapper.instance().renderForecastBlock()).toBeNull()
      })
    })
  })
})
